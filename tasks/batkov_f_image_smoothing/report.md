...
- реализовать вспомогательную инфраструктуру для чтения/записи изображений и построения гауссовых ядер;
...
На вход алгоритма подаётся цветное либо одноканальное изображение, загружаемое из файла.
...
Класс `Image` инкапсулирует:

- размеры изображения `width_`, `height_`;
- количество каналов `channels_` (1, 3 или 4);
- вектор байтов `std::vector<uint8_t> data_`, хранящий пиксели в формате (RGB[A]).

Задача: по входному изображению построить новое изображение, на которое применён **оператор сглаживания** (гауссовое размытие) с фиксированным ядром 5×5. Полученное изображение должно быть более сглаженным, что проверяется детектором `LaplacianVarianceSmoothDetector`.
...
Гауссово ядро размером `size × size` и параметром \( \sigma \) генерируется классом `GaussianKernelFabric`:
...
## 5. Детектор "размытия" (LaplacianVarianceSmoothDetector)

Класс `LaplacianVarianceSmoothDetector` (`common/src/smooth_image_detector.cpp`) используется в тестах для проверки, что результат действительно стал более сглаженным.
...
4. Если дисперсия меньше порога `threshold_`, изображение считается **сглаженным**.

Метод `IsSmooth(const Image&)` возвращает булево значение, которое используется в тестах как критерий корректности работы сглаживания.

## 6. Детали реализации и структура проекта
...
│   ├── include
│   │   ├── common.hpp
│   │   ├── gaussian_kernel_fabric.hpp
│   │   ├── image.hpp
│   │   └── smooth_image_detector.hpp
│   └── src
│       ├── gaussian_kernel_fabric.cpp
│       ├── image.cpp
│       └── smooth_image_detector.cpp
├── data
│   ├── test1.jpeg ... test5.jpeg
├── info.json
├── mpi
│   ├── include
│   │   └── ops_mpi.hpp
...
- `Image` — загрузка изображения из файла (через `stb_image`), хранение и доступ к пикселям;
- `GaussianKernelFabric` — генерация гауссова ядра фиксированного размера;
- `LaplacianVarianceSmoothDetector` — детектор "размытия" на основе дисперсии лапласиана;
...
1. **Функциональные тесты** (`tests/functional/main.cpp`):
   - используются изображения `test1.jpeg` … `test5.jpeg` разного содержания и уровня шума;
   - для каждого изображения задаётся свой порог `threshold` для детектора размытия;
   - для каждого теста запускаются обе реализации: SEQ и MPI.

2. **Тесты производительности** (`tests/performance/main.cpp`):
   - используется изображение `test1.jpeg` как репрезентативный пример;
...
- Функциональные тесты проверяют, что результат сглаживания удовлетворяет критерию `IsSmooth(output)` для заданного порога.
...
- Дополнительная проверка визуально (по содержимому файлов-результатов, при необходимости) подтверждает, что сглаживание устраняет мелкие детали и шум, сохраняя крупные структуры.
...
3. **Инфраструктура тестирования**: класс `Image` для работы с файлами изображений, генератор гауссова ядра и детектор размытия на основе дисперсии лапласиана.
4. **Функциональные и производительные тесты**, демонстрирующие корректность и исследующие поведение алгоритма при разных режимах запуска.
